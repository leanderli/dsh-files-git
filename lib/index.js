 ...
 *                                          (hash/name/email/date/refs/subject)
 ...
 * gave up on stops spending host CPU / disk I/O instead of running to
 * completion. */
...
async function dispatch(endpoint, payload, env) {
	...
	switch (endpoint) {
		...
		case "list": {
			return await listDirectory(repo, typeof payload.path === "string" ? payload.path : "", signal);
		}
		case "stat": {
...
function apply(ctx, config) {
	...
	// unbounded git processes / directory walks inside this single Node
	// process and starve the rest of DSH. Excess requests fail fast with
	// `busy` — the client surfaces a retryable error, and its own in-flight
	// dedup + loadDir guard prevent a re-storm. 12 leaves normal use
	// untouched (initial load is 6, a full refresh is 4).
	...
	ctx.connection.register(
		...
		async (endpoint, payload, signal) => {
			if (activeRpc >= MAX_CONCURRENT_RPCS) return fail("busy", "服务器繁忙，请稍后重试");
			activeRpc++;
			try {
