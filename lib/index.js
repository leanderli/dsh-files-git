 ...
 *     user-private ~/.dsh-files-git dir: read → health check → reuse, else
 *     request old shutdown, spawn, poll until healthy; version + config
 *     fingerprint trigger a rotation on upgrade/config change), and
...
/** floor(≤0) yields null (unconfigured): a 0 would otherwise join the
 * fingerprint here while the sidecar's positiveNumber() treats it as absent
 * — the two sides would then compute DIFFERENT runtime-file names and the
 * host would poll a file that never appears. */
const byteCap = (value) => {
	const n = Number.isFinite(value) && value > 0 ? Math.floor(value) : null;
...
/**
 * Probe an instance: "ok" (same version, healthy), "refused" (nothing
 * listening — definitively gone), or "unreachable" (timeout/reset: the
 * sidecar may be ALIVE but busy, e.g. a big export blocking its loop).
 * The distinction matters: only "refused" justifies requestShutdown —
 * killing an unreachable-but-alive instance would hard-terminate in-flight
 * git mutations and leave .git/index.lock behind (the exact historical
 * pain this architecture exists to cure).
 */
async function probeInstance(entry, timeoutMs) {
	try {
		const { status, data } = await fetchJson(`http://127.0.0.1:${entry.port}/health`, entry.token, "GET", timeoutMs);
		if (status === 200 && data?.ok === true && data?.version === PLUGIN_VERSION) return "ok";
		return "foreign";
	} catch (error) {
		return isConnectRefused(error) ? "refused" : "unreachable";
	}
}

/**
 * Probe with the deadline discipline above: an "unreachable" instance gets
 * a couple of extra chances (a busy sidecar comes back within seconds)
 * before we fall back to treating it as dead.
 */
async function instanceHealthy(entry, timeoutMs) {
	let verdict = await probeInstance(entry, timeoutMs);
	if (verdict === "unreachable") {
		for (let i = 0; i < 2 && verdict === "unreachable"; i++) {
			await new Promise((r) => setTimeout(r, 400));
			verdict = await probeInstance(entry, timeoutMs);
		}
	}
	return verdict;
}
 ...
 * (The runtime file records a random loopback port; if that port has since
 * been reused by some OTHER local process, this POST is an unauthenticated
 * probe of it. The response is ignored and such a process answers 404 to an
 * unknown path — nothing beyond that one request happens.)
 */
...
function apply(ctx, config) {
	...
	async function ensureService() {
		...
		if (entry && entry.version === PLUGIN_VERSION && entry.fingerprint === fingerprint) {
			const verdict = await instanceHealthy(entry, 1200);
			if (verdict === "ok") {
				ensureFailedAt = 0;
				service = entry;
				return;
			}
			// "refused": definitely gone → reclaim. "foreign": wrong
			// version/config → rotate. "unreachable": only after the retries
			// inside instanceHealthy — treat as dead but log it loudly (the
			// busy-but-alive case we tried to spare may have been real).
			if (verdict === "unreachable") slowLog(`service ${entry.pid} unreachable after retries — rotating anyway`);
			await requestShutdown(entry);
		} else if (entry) {
			await requestShutdown(entry);
		}
		try {
			...
			// stderr is piped (not ignored) so an import-time crash — the
			// module never reaching main(), e.g. a syntax error — leaves a
			// trace here instead of vanishing with stdio:"ignore".
			...
			child.on("error", (error) => {
				// Async spawn failure (EMFILE/ENFILE/ENOMEM…): without this
				// listener EventEmitter re-throws as uncaughtException and
				// takes the WHOLE DSH main process down. We only log — the
				// poll loop below fails the boot in an orderly way.
				slowLog(`spawn service failed: ${String(error?.message ?? error)}`);
			});
			child.on("exit", (code) => {
				if (code !== 0 && code !== null) slowLog(`service exited prematurely code=${code} stderr: ${stderrTail.trim().slice(0, 300)}`);
			});
		} catch (error) {
			slowLog(`spawn service failed: ${String(error?.message ?? error)}`);
		}
		...
		while (Date.now() < deadline) {
			...
			if (e && e.version === PLUGIN_VERSION && e.fingerprint === fingerprint && (await probeInstance(e, 800)) === "ok") {
				ensureFailedAt = 0;
				service = e;
				return;
			}
	...
	async function callService(endpoint, payload, signal) {
		// A pre-aborted signal never fires the listener below — without this
		// check a dead request would still run its full proxy round-trip.
		...
		let timedOut = false;
		const timer = setTimeout(() => { timedOut = true; controller.abort(); }, SERVICE_REQUEST_TIMEOUT_MS);
		try {
			...
			// OUR 200s deadline fired (not the caller giving up): the git
			// tree was already killed by the abort fan-out — say "timeout",
			// not the misleading "请求已取消".
			if (timedOut) return fail("git-timeout", `git 服务处理超时（${Math.round(SERVICE_REQUEST_TIMEOUT_MS / 1000)}s），请缩小范围后重试`);
			throw error;
		} finally {
			clearTimeout(timer);
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			// Fail CLOSED: if the connection service ever lacks
			// requestRejection (older/newer DSH), refuse the route rather
			// than serve token-injected raw transfers unfenced.
			if (typeof ctx.connection?.requestRejection !== "function") {
				sendJson(res, 503, fail("service-unavailable", "git 文件通道不可用（缺少请求围栏）"));
				return;
			}
			...
			// Same short-circuit as the RPC channel: once the service is up,
			// skip the re-probe entirely. Unconditional ensureOnce here used
			// to health-probe on EVERY transfer — and a busy sidecar (large
			// export) could fail that probe and get rotated mid-work.
			if (!service) {
				try {
					await ensureOnce();
				} catch (error) {
