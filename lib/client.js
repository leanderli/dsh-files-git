...
`;
		...
		function useGit(cwd) {
			...
			useEffect(() => {
				...
				const tick = () => {
					...
					// (slid out of view): no point spawning 4 git processes the
		...
		function cmCore() {
			if (!cmCorePromise) {
				...
				// A rejected aggregate must NOT poison the whole page session
				// (the idle warmup calls this early; one CDN hiccup at that
				// moment would otherwise kill every later preview/edit until
				// reload). Same eviction contract as cmImport: the CURRENT
				// callers still get this rejection, the NEXT call retries.
				p.catch(() => { if (cmCorePromise === p) cmCorePromise = null; });
				cmCorePromise = p;
			}
			return cmCorePromise;
		}
		/** Pre-import the CodeMirror core (+ dark theme + the grammars for the
		 * given file extensions, capped) so the first preview/edit mounts from
		 * the warm cache instead of the network. Fire-and-forget: failures are
		 * already retried by cmImport's cache eviction on next real use. */
		function cmWarmup(exts = []) {
			cmCore().catch(() => {});
			if (typeof document !== "undefined" && document.body.hasAttribute("data-ds-dark-theme")) cmImport("theme-one-dark").catch(() => {});
			const seen = new Set();
			for (const ext of exts) {
				const name = CM_LANG_FOR_EXT[ext];
				if (!name || seen.has(name) || seen.size >= 4) continue;
				seen.add(name);
				cmImport(`lang-${name}`).catch(() => {});
			}
		}
		/**
		 * Create a CodeMirror editor inside `hostEl`. opts: { doc, ext, dark,
		 * readOnly, onChange }. Returns the EditorView. The one-dark theme is
		 * applied for dsh's dark mode; the default light theme otherwise.
		 * `readOnly: true` renders a non-editable view — the SAME renderer the
		 * edit mode uses, so code previews and editing share identical
		 * indentation and highlighting (keymaps stay: search works, editing
		 * commands are no-ops in read-only).
		 */
			...
			const viewInst = new view.EditorView({
				state: state.EditorState.create({
					...
					extensions: [
						...
						...(langExt ? [langExt] : []),
						...(readOnly ? [state.EditorState.readOnly.of(true), view.EditorView.editable.of(false)] : []),
						view.keymap.of([
		...
		function FileBrowser({ cwd, status, refreshTick, onEdited }) {
			...
			// Read-only CodeMirror preview for code files: the SAME renderer edit
			// mode uses (CDN CodeMirror 6 + per-language packs), so indentation,
			// wrapping and token highlighting are identical to the editor. The
			// old hand-rolled tokenizer mis-rendered leading whitespace on long
			// lines and at 30 KB chunk boundaries; CM renders line-based, so
			// indentation is always exact. Mutually exclusive with edit mode
			// (pvEdit); destroyed on preview change, mode switch or unmount.
			useEffect(() => {
				if (!preview || preview.kind !== "code" || pvMode !== "preview" || pvEdit || !pvCmHostRef.current) { setPvCmBusy(false); return; }
				let alive = true;
				// The busy hint is DELAYED (150ms): once the CDN modules are in
				// the page cache the mount finishes within a few microtasks, and
				// an unconditional hint would flash "loading" on EVERY file
				// open. Only genuinely slow mounts (cold cache / huge document)
				// ever show it. The warmup effect below keeps the cache hot.
				const hintTimer = window.setTimeout(() => { if (alive) setPvCmBusy(true); }, 150);
				setPvCmErr(null);
				const dark = typeof document !== "undefined" && document.body.hasAttribute("data-ds-dark-theme");
				cmCreateEditor(pvCmHostRef.current, {
					doc: preview.text || "",
					ext: extOf(preview.name),
					dark,
					readOnly: true,
					...
					if (!alive) { view.destroy(); return; }
					pvCmRef.current = view;
					setPvCmBusy(false);
				}).catch((err) => {
					window.clearTimeout(hintTimer);
					// CDN unreachable → fall back to the plain source view (one
					// click away in source mode); never leave a dead pane.
					if (!alive) return;
					setPvCmBusy(false);
				...
				return () => {
					alive = false;
					window.clearTimeout(hintTimer);
					if (pvCmRef.current) { pvCmRef.current.destroy(); pvCmRef.current = null; }
				};
			}, [preview, pvMode, pvEdit, pvCmTick]);
			// Warm the CodeMirror CDN cache while the user merely browses: the
			// core (6 packages) plus the grammars this workspace actually uses
			// (top extensions in the file index) are imported on idle, so the
			// first code preview usually mounts with zero network wait.
			useEffect(() => {
				if (!allFiles) return;
				const counts = new Map();
				for (const p of allFiles) {
					const e = extOf(p);
					if (CM_LANG_FOR_EXT[e]) counts.set(e, (counts.get(e) ?? 0) + 1);
				}
				const exts = [...counts.entries()].sort((a, b) => b[1] - a[1]).map(([e]) => e);
				const run = () => cmWarmup(exts);
			...
			const previewView = () => {
				...
				if (pvEdit) {
					...
					// Preview mode: read-only CodeMirror — the SAME renderer as
					// edit mode, so indentation/highlighting are identical. On
					// CDN failure (offline) the pane falls back to the plain
