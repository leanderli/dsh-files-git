...
/* DSH settings spec: groups separated by hairline rules, 14px titles, 12px hints */
.dgp-setGroup{border-bottom:.5px solid var(--dsw-alias-border-l2);display:flex;flex-direction:column;gap:8px;padding:16px 0}
.dgp-setGroupHead{display:flex;flex-direction:column;gap:2px}
.dgp-setGroupTitle{font-size:14px;font-weight:400;line-height:22px;color:var(--dsw-alias-label-primary)}
.dgp-setGroupHint{font-size:12px;line-height:18px;color:var(--dsw-alias-label-secondary)}
/* Rounded "cube" option buttons (DSH appearance-row treatment) */
.dgp-setCubes{display:flex;flex-wrap:wrap;align-items:stretch;gap:8px}
.dgp-setCube{box-sizing:border-box;font:inherit;font-size:13px;line-height:20px;color:var(--dsw-alias-label-primary);cursor:pointer;background:0 0;border:.5px solid var(--dsw-alias-border-l4);border-radius:20px;padding:8px 20px;display:inline-flex;align-items:center;justify-content:center;white-space:nowrap}
.dgp-setCube:hover:not([data-active="true"]){background:var(--dsw-alias-interactive-bg-hover)}
.dgp-setCube[data-active="true"]{background:var(--dsw-alias-bg-module-platform, color-mix(in srgb,var(--dsw-alias-state-business-primary) 14%,transparent));border-color:var(--dsw-static-neutral-bluish-400, var(--dsw-alias-state-business-primary));color:var(--dsw-alias-label-primary)}
...
.dgp-histEntry{flex-direction:column;align-items:stretch;gap:1px;padding-top:5px;padding-bottom:5px}
...
/* Compact change-tree rows: tighter vertical rhythm. The status label sits
   right after the file name (moved from the far right), so it stays visible
   at every depth — deep rows only shrink the action buttons. */
...
.dgp-pane{min-width:0;overflow:auto;border:1px solid var(--dsw-alias-border-l1);border-radius:10px;background:color-mix(in srgb,var(--dsw-alias-bg-base) 98%,transparent);padding:6px;display:flex;flex-direction:column;gap:1px;transition:background .2s;contain:content}
...
.dgp-root[data-narrow="true"] .dgp-header{gap:8px;padding:calc(10px + env(safe-area-inset-top,0px)) 14px 8px}
.dgp-root[data-narrow="true"] .dgp-tabs{padding:0 14px}
/* !important: .dgp-body carries inline padding from FilePanelBody. */
.dgp-root[data-narrow="true"] .dgp-body{padding:10px 12px calc(14px + env(safe-area-inset-bottom,0px)) !important}
...
.dgp-root[data-narrow="true"] .dgp-crumbDir{display:none}
...
`;
		...
		const zh = {
			...
			"set.themeHint": "作用于 Monaco 预览 / 编辑 / Diff 视图；「跟随面板」随 DSH 明暗自动切换（深色 HC Black · 浅色 HC Light），切换立即生效。",
			"set.themeAuto": "跟随面板",
		...
		const en = {
			...
			"file.actionsTitle": "More actions",
			...
			"set.themeHint": "Applies to Monaco preview / editing / diff views. “Follow panel” tracks the DSH light/dark scheme (dark: HC Black · light: HC Light). Changes apply instantly.",
			"set.themeAuto": "Follow panel",
		...
		// Rendering: Monaco diff editor (sidecar-hosted) when available and the
		// unified text is reconstructable; the legacy row renderer stays as a
		// byte-identical fallback (loader failure, non-loopback page, binary /
		// truncated diffs). The stats chips always come from diffRows().
		const DiffPane = React.memo(function DiffPane({ diff, onShowDiff, onClose }) {
			...
			// Mount ONCE per pane lifetime; diff switches reuse the editor via
			// inst.update() (no dispose/remount churn between file selections).
			const latest = useRef({ text: diff.text, path: diff.path });
			latest.current = { text: diff.text, path: diff.path };
			useEffect(() => {
				if (diff.text.trim() === "") return undefined;
				let alive = true;
				let timer = null;
				const retry = () => {
					// The first attempt often races the host element's layout
					// (pane just became visible) — one deferred retry covers it.
				...
				const run = async () => {
					...
					try {
						...
						// loader/sidecar failure: retry once, then give up to the
						// legacy renderer for THIS diff (a later diff re-runs run).
						if (!alive) return;
						if (timer === null) { timer = -1; retry(); } else setFallback(true);
					}
				};
				run();
				return () => {
			...
			return h("div", { className: "dgp-gitCard dgp-diffCard" },
				...
				parsed === null ? h("div", { className: "dgp-empty", style: { flex: 1, display: "flex", alignItems: "center", justifyContent: "center" } }, t("diff.noContent"))
					: h(React.Fragment, null,
						...
						fallback ? h("div", { className: "dgp-diff" },
							parsed.rows.slice(0, 5000).map((r) => h("div", { key: r.key, className: "dgp-diffLine" },
								h("span", { className: "dgp-diffSpan" + (r.cls ? " " + r.cls : "") },
									r.segs ? r.segs.map((sg, k) => h("span", { key: k, className: sg.t === "same" ? "dgp-diffWordSame" : sg.t === "add" ? "dgp-diffWordAdd" : "dgp-diffWordDel" }, sg.s)) : r.text
								)
							))
		...
		const HistoryBlock = React.memo(function HistoryBlock({ log, currentHash, onShowCommit, setConfirm, runOp, cwd }) {
			...
			const entries = log.lines.map((raw) => {
				if (typeof raw === "string") {
					const sp = raw.indexOf(" ");
					const h = sp === -1 ? raw : raw.slice(0, sp);
					return { h, hash: h, subject: sp === -1 ? "" : raw.slice(sp + 1), refs: "", name: "", email: "", date: "" };
				}
				return raw;
			});
			...
			return h(React.Fragment, null,
				h("div", { className: "dgp-gitCard dgp-histCard", ref: histRef },
					...
					h("div", { className: "dgp-gitScroll dgp-histList" }, entries.slice(0, logLimit).map((e) => {
		...
		function GitView({ git }) {
			...
			return h(React.Fragment, null,
				...
				h("div", { className: "dgp-gitTop" },
					...
					h("div", { className: "dgp-opBar" },
						h("div", { className: "dgp-actions" },
							...
							// Mutations use the global busy lane, reads (diff/show)
							// the readBusy lane — the hint covers both.
							(git.busy !== null || git.readBusy !== null) ? h("span", { className: "dgp-hint" }, t("gv.busy", { op: t(git.busy ?? git.readBusy) })) : null
						),
				...
				: h(React.Fragment, null,
				h("div", { className: "dgp-gitWork", "data-diff": git.diff ? "true" : "false", ref: narrow ? undefined : diffRef, style: git.diff && !narrow ? { gridTemplateColumns: `minmax(0,${diffShare.toFixed(3)}fr) 8px minmax(0,${(1 - diffShare).toFixed(3)}fr)` } : undefined },
					// left column: working-tree changes (commit row now lives in
					// the action bar above, so the list reaches down to history).
					// Narrow drill-down: hidden while the diff screen shows.
					(!narrow || !git.diff) ? h("div", { className: "dgp-gitCol" },
						s ? h(ChangeList, {
 ...
 * The HC preferences resolve to DERIVED themes ("dsh-dark"/"dsh-light", built
 * on hc-black/hc-light) that restore the classic green/red LINE BACKGROUND
 * blocks for diffs — the stock HC themes replace backgrounds with borders
 * (a11y style), which makes changed lines hard to spot. */
function resolveMonacoTheme() {
	let pref = "auto";
	try { pref = localStorage.getItem(MONACO_THEME_KEY) || "auto"; } catch { /* private mode */ }
	if (pref === "vs" || pref === "vs-dark") return pref;
	const dark = pref === "hc-black" ? true : pref === "hc-light" ? false : isDshDark();
	return dark ? "dsh-dark" : "dsh-light";
}

/** Register the derived HC themes (called once per Monaco load). */
function defineDshThemes(monaco) {
	monaco.editor.defineTheme("dsh-dark", {
		base: "hc-black", inherit: true, rules: [],
		colors: {
			"diffEditor.insertedLineBackground": "#00C85324",
			"diffEditor.removedLineBackground": "#FF525224",
			"diffEditor.insertedTextBackground": "#00C8533D",
			"diffEditor.removedTextBackground": "#FF52523D"
		}
	});
	monaco.editor.defineTheme("dsh-light", {
		base: "hc-light", inherit: true, rules: [],
		colors: {
			"diffEditor.insertedLineBackground": "#00C8531F",
			"diffEditor.removedLineBackground": "#FF52521F",
			"diffEditor.insertedTextBackground": "#00C85338",
			"diffEditor.removedTextBackground": "#FF525238"
		}
	});
}
...
async function mountMonacoEditor(hostEl, { value, path, readOnly }) {
	...
	return {
		...
		const FileRow = React.memo(function FileRow({ entry, path, selected, status, tapOpens, onSelect, onOpenDir, onOpenPreview, onOpenFolder, onCopyPath, onCopyName }) {
		...
		function FileBrowser({ cwd, status, refreshTick, onEdited }) {
			...
			const loadDir = useCallback(async (rel) => {
				if (loadingDirsRef.current.has(rel)) return;   // already in flight — never duplicate
			...
			// Auto-load a pane whose directory is missing or still loading. The
			// loadingDirsRef guard makes these effects idempotent, so the `dirs`
			// dependency can no longer feed an infinite re-request loop.
			...
			const renderList = (path, opts = {}) => {
				...
				const row = (entry) => h(FileRow, { key: entry.path, entry, path, selected: sel === entry.path, status, tapOpens: narrow, onSelect: () => setSel(entry.path), onOpenDir: openDir, onOpenPreview: openPreview, onOpenFolder, onCopyPath, onCopyName });
				return h(React.Fragment, null,
			...
			return h(React.Fragment, null,
				...
				h("div", { className: "dgp-body", "data-tab": tab, style: { overflow: "auto", padding: "12px 16px 16px", gap: 10 } },
					tab === "settings" ? h(SettingsView, { onSetMax })
						: (tab === "files" || git.isRepo === false ? h(FileBrowser, { cwd, status: git.status, refreshTick, onEdited: () => { git.refresh({ full: true }); setRefreshTick((t) => t + 1); } }) : h(GitView, { git }))
				)
			);
		});
		...
		// death also kills the top-center handle (the only re-entry while
		// suspended), stranding the panel for good.
		...
		function FilePanelOverlay() {
			...
			useEffect(() => {
				...
				// suspend button (and the top handle) remain the exits.
				if (narrow) return;
			...
			// (dialog shell, handle, an effect-driven re-render) must degrade
		...
		const inject = ["slots", "sessions", "workspaces", "locale", "connection"];
			...
			try {
				...
				if (!alreadyWrapped && connection?.rpc && typeof connection.rpc.call === "function") {
					...
					if (!desc || desc.writable || desc.set) {
						...
						// module instance and intercept with stale stores.
						ctx.effect(() => async () => {
							try {
