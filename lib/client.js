		...
		function computeLogLanes(entries) {
			...
			return entries.map((e) => {
				...
				if (dot === -1) {
					...
					// fan-in from beyond it): open a new lane; at capacity,
					// collapse into the last lane instead of growing further.
					dot = Math.min(lanes.length, GRAPH_LANES_MAX - 1);
				...
				const curves = [];
				if (p0) lanes.splice(dot, 0, { sha: p0, color: consumed.color });
				let insertAt = dot + 1;
				...
				if (p0) segs.push({ a: dot, b: dot, half: "bottom", color: consumed.color });
