...
`;
		...
		function useGit(cwd) {
			...
			const refresh = useCallback(async ({ quiet = false, full = false } = {}) => {
				...
				try {
					...
					// healthy.
					const [stR, lgR, cfgR, brR] = await Promise.allSettled([
					...
					if (stR.status === "rejected") throw stR.reason;
					const st = stR.value;
		...
		function BranchSelector({ git }) {
			...
			return h("div", { className: "dgp-branchTab", "data-open": branchOpen ? "true" : "false" },
				...
				branchOpen && branchPos ? portal(h("div", { className: "dgp-branchPop", style: { left: branchPos.left, top: branchPos.top }, onClick: (e) => e.stopPropagation() },
...
function ensureMonaco() {
	...
	monacoPromise = (async () => {
		...
		// 2) Blob proxy for cross-origin workers. The blob URL is created
		// ONCE and reused: Monaco spawns several workers (editor + one per
		// language service), and a createObjectURL per getWorkerUrl call
		// leaked a blob entry for each — one URL serves them all (it stays
		// valid for the document's lifetime; nothing here ever revokes it).
