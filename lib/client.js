...
`;
		...
		async function filesTransport() {
			const info = svcInfo ?? await bootstrapServiceInfo();
			return info
				? { base: `http://127.0.0.1:${info.port}`, token: info.token }
				: { base: "/git-api-files", token: null };
		}
		const filesUrl = (base, route, params) => {
			const qs = new URLSearchParams(params ?? {}).toString();
			return `${base}/files/${route}${qs ? `?${qs}` : ""}`;
		};
		...
		async function exportZipDownload({ repo, paths, name, signal }) {
			...
			for (let attempt = 0; attempt < 2; attempt++) {
				...
				const blob = await res.blob();
				triggerDownload(blob, value.name);
				return value;
			}
...
async function mountMonacoEditor(hostEl, { value, path, readOnly }) {
	...
	return {
		...
		function FileBrowser({ cwd, status, refreshTick, onEdited }) {
			...
			useEffect(() => { setDirs({}); setLeftPath(null); setRightPath(""); setPreview(null); setSel(null); setSelSet({}); setBrowseError(null); loadDir(""); }, [loadDir]);
			...
			// bar exits multi mode only then).
			const exportStart = useCallback(async (paths, name) => {
				...
				try {
					await exportZipDownload({ repo: cwd, paths, name });
					return true;
				} catch (err) {
