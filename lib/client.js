...
`;
		...
		const zh = {
			...
			"set.themeHint": "作用于 Monaco 预览 / 编辑视图；「跟随面板」随 DSH 明暗自动切换（深色 HC Black · 浅色 HC Light），切换立即生效。Diff 视图不随主题：固定红/蓝色块（删除=红、更改=蓝）。",
			"set.themeAuto": "跟随面板",
		...
		const en = {
			...
			"set.themeHint": "Applies to Monaco preview / editing views. “Follow panel” tracks the DSH light/dark scheme (dark: HC Black · light: HC Light). Changes apply instantly. Diff views are theme-independent: fixed red/blue blocks (removed=red, changed=blue).",
		...
		const MonacoDiffHost = React.memo(function MonacoDiffHost({ text, path, children }) {
			...
			return h("div", { className: "dgp-gitCard dgp-diffCard" },
				...
				parsed === null ? h("div", { className: "dgp-empty", style: { flex: 1, display: "flex", alignItems: "center", justifyContent: "center" } }, t("diff.noContent"))
					: h(React.Fragment, null,
						...
						h(MonacoDiffHost, { text: diff.text, path: diff.path }, diffRowsView(parsed, diff.truncated))
					)
			);
		});
					...
					(!narrow || filePane) ? h("div", { className: "dgp-gitCard dgp-diffCard" },
						...
						d.loadingFile ? h("div", { className: "dgp-empty", style: { flex: 1, display: "flex", alignItems: "center", justifyContent: "center" } }, t("common.loading"))
							: fileParsed ? h(React.Fragment, null,
								...
								h(MonacoDiffHost, { text: d.text, path: d.file }, diffRowsView(fileParsed, d.truncated))
							)
