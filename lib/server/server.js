 ...
 *
 * Lifecycle:
 *   - Singleton via a runtime file `~/.dsh-files-git/dsh-files-git-service.json`
...
import { appendFileSync, closeSync, createReadStream, createWriteStream, existsSync, linkSync, mkdirSync, openSync, readFileSync, readSync, readdirSync, realpathSync, renameSync, statSync, unlinkSync, watch, writeFileSync } from "node:fs";
import { readdir, stat, writeFile } from "node:fs/promises";
import { tmpdir } from "node:os";
import { basename, delimiter as pathDelimiter, dirname, isAbsolute, join, resolve, sep } from "node:path";
import { pipeline } from "node:stream/promises";
import { fileURLToPath } from "node:url";
import { configFingerprint, runtimeBaseDir, runtimeFilePath } from "./shared.js";
...
/** Concurrent byte-transfer routes (/files/*) and unauthenticated vendor
 * reads: one panel needs a handful; anything beyond signals a runaway or
 * abusive local client, and the raw channel must not pin unbounded sockets. */
const MAX_CONCURRENT_TRANSFERS = 16;
const MAX_CONCURRENT_VENDOR = 32;
/** Upload temp files live next to their target (same-dir atomic rename):
 *  `<name>.dshup-<hex>` — note the SUFFIX form: the anchor must not require
 *  the whole name to be the marker, or hide/sweep/skip all go dead. A
 *  crashed sidecar orphans them; listings sweep stale ones and hide all of
 *  them, exports skip them. */
const UPLOAD_TEMP_RE = /\.dshup-[0-9a-f]{8}$/;
const UPLOAD_TEMP_SWEEP_MS = 6 * 60 * 60 * 1000;
 ...
 * for the platform's git binary, skipping `.git-ai` shim directories so the
 * panel drives plain git semantics (a wrapper could inject prompts or alter
 * behavior). PATH splitting is delimiter-aware: `;` only on Windows, `:`
 * on POSIX (a `;`-only split would never resolve on Linux/macOS). */
function resolveGitPath(configured) {
	...
	for (const dir of pathDirs) {
		...
		const candidate = join(dir, gitBin);
		try {
 ...
 * On POSIX the same holds, so runGit spawns the child detached (own process
 * group) and we signal the NEGATIVE pid to reap the whole group.
 */
...
/**
 * Whitelist for user-supplied rev/branch strings that are placed in argv
 * BEFORE `--`: a value like `--output=<path>` would otherwise be parsed by
 * git as an option (turning a read-only "view this commit" into a
 * file-write primitive). Accepts hex object ids (4-40 chars), `HEAD` with
 * `~`/`^` suffixes, and refname-shaped tokens — unicode branch names are
 * legal, so the check rejects git's own forbidden refname set plus a
 * leading `-`/`/` instead of trying to whitelist characters.
 */
const REV_SHA_RE = /^[0-9a-fA-F]{4,40}$/;
function isValidRev(value) {
	if (typeof value !== "string") return false;
	const s = value.trim();
	if (s === "") return false;
	if (REV_SHA_RE.test(s)) return true;
	if (/^HEAD(?:[~^]\d{0,3})*$/.test(s)) return true;
	if (s.charCodeAt(0) === 45 /* - */ || s.charCodeAt(0) === 47 /* / */) return false;
	// git check-ref-format's forbidden set (plus backslash) and shape rules.
	if (/[\x00-\x20\x7f~^:?*[\]\\]/.test(s)) return false;
	if (s.includes("..") || s.endsWith(".lock") || s.endsWith("/") || s === "@") return false;
	return true;
}
 ...
 * { code, timedOut, stdout, stderr, stdoutCapped } — never throws for a
 * non-zero exit.
 */
function runGit(gitPath, repo, args, { timeoutMs = STATUS_TIMEOUT_MS, signal } = {}) {
	return new Promise((resolve) => {
		// The caller already gave up: don't even spawn (a pre-aborted signal
		// would not fire the abort listener, and the work would be wasted).
		if (signal?.aborted) {
			resolve({ code: -1, timedOut: false, stdout: "", stderr: "aborted", stdoutCapped: false });
			return;
		}
		let child;
		try {
			// core.quotePath=false: emit RAW UTF-8 paths. Git's default quotes
			// non-ASCII names as octal escapes ("文件" → "\346\226\207\344\273\266"),
			// which would land verbatim in the change list; LC_ALL=C below makes
			// that escaping certain without this override. Applies to EVERY git
			// call (status/ls-files/log/show/diff all carry paths).
			// POSIX: `detached` puts the child in its OWN process group so
			// killTree can reap the whole tree (helpers holding index.lock
			// otherwise survive — see killTree). Harmless on Windows.
			child = spawn(gitPath, ["-c", "core.quotePath=false", "-C", repo, ...args], {
				env: {
					...process.env,
			...
			resolve({ code: -1, timedOut: false, stdout: "", stderr: String(error), stdoutCapped: false });
			return;
		}
		// Collect Buffers and decode ONCE at the end: decoding each chunk
		// separately would split multi-byte UTF-8 characters at chunk
		// boundaries into U+FFFD (中文文件名乱码 on >64KB outputs). The cap is
		...
		const finish = (code) => {
			...
			resolve({
				...
				stdoutCapped: stdoutBytes >= MAX_OUTPUT_BYTES
			});
		};
		const onAbort = () => {
		...
		const onTimeout = () => {
			timedOut = true; // distinguish "we killed it for time" from a crash
			killTree(child);
		};
...
function resolveUnder(root, rel) {
	...
	// Drive roots resolve WITH their trailing separator ("D:\"); appending
	// another would make every child fail the prefix test ("D:\\" never
	// prefixes "D:\foo") — normalize before comparing.
	...
	const baseRealPrefix = baseReal.endsWith(sep) ? baseReal : baseReal + sep;
	if (targetReal !== baseReal && !targetReal.startsWith(baseRealPrefix)) {
...
function resolveUnderForCreate(root, rel) {
	...
	// Control characters are rejected everywhere; the Windows-reserved set
	// (`*?"<>|:` and backslash) only where it applies — a POSIX DSH host must
	// still accept legal names like "a:b.txt".
	if (/[\x00-\x1f]/.test(rel)) return fail("invalid-path", "路径含控制字符");
	if (process.platform === "win32" && /[*?"<>|:\\]/.test(rel)) return fail("invalid-path", "路径含非法字符");
	const base = resolve(root);
	const target = resolve(base, rel.replace(/\//g, sep));
	const basePrefix = base.endsWith(sep) ? base : base + sep;
	if (target !== base && !target.startsWith(basePrefix)) {
	...
	const baseRealPrefix = baseReal.endsWith(sep) ? baseReal : baseReal + sep;
	if (parentReal !== baseReal && !parentReal.startsWith(baseRealPrefix)) {
		return fail("invalid-path", "路径超出工作区范围（符号链接）");
	}
...
async function listDirectory(root, rel, signal) {
	...
	try {
		...
		if (signal?.aborted) return fail("aborted", "请求已取消");
		// Opportunistic cleanup: upload temps orphaned by a crashed sidecar
		// would otherwise linger in listings and git status forever. Only
		// files past the sweep age go — a LIVE upload's temp is recent.
		for (const dirent of dirents) {
			if (!UPLOAD_TEMP_RE.test(dirent.name) || !dirent.isFile()) continue;
			try {
				if (statSync(join(target, dirent.name)).mtimeMs <= Date.now() - UPLOAD_TEMP_SWEEP_MS) {
					unlinkSync(join(target, dirent.name));
				}
			} catch { /* raced with another sweep */ }
		}
		// Upload temps never show in listings (live or orphaned).
		const visible = dirents.filter((d) => !UPLOAD_TEMP_RE.test(d.name));
		// Cap huge directories so one listing never floods the response.
		const MAX_ENTRIES = 4000;
		const capped = visible.slice(0, MAX_ENTRIES);
		...
		return ok({ entries, truncated: visible.length > MAX_ENTRIES });
	} catch (error) {
...
function readFileUnder(root, rel, maxBytes, signal) {
	...
	try {
		...
		const buffer = readPrefix(target, cap);
		if (signal?.aborted) return fail("aborted", "请求已取消");
		const sample = buffer.subarray(0, Math.min(8192, buffer.length));
		if (sample.includes(0)) {
			return ok({ text: "", binary: true, truncated: false, size: st.size });
		}
...
/**
 * Read at most `cap + 1` bytes from the front of a file — previews never
 * need more, and a whole-file readFileSync would balloon the sidecar's RSS
 * (or throw ERR_FS_FILE_TOO_LARGE) when the tree contains multi-GB
 * dataset/log files. The +1 lets callers see "there was more" cheaply.
 */
function readPrefix(target, cap) {
	const want = cap + 1;
	...
	try {
		let total = 0;
		while (total < want) {
			const n = readSync(fd, buffer, total, want - total, total);
			if (n <= 0) break;
			total += n;
		}
...
async function gitRun(gitPath, repo, args, options) {
	if (options?.signal?.aborted) return fail("aborted", "请求已取消");
	const argv = options?.lockFree === true ? ["--no-optional-locks", ...args] : args;
	for (let attempt = 0; attempt < 2; attempt++) {
		const result = await runGit(gitPath, repo, argv, options);
		if (result.code === 0) return ok({ code: result.code, stdout: result.stdout, stderr: result.stderr, stdoutCapped: result.stdoutCapped === true });
		// Killed for running past the deadline: report the timeout plainly
		// and NEVER retry (a POSIX SIGKILL leaves code<0 with empty output,
		// which the crash-retry below would otherwise double-wait on).
		if (result.timedOut) {
			return fail("git-timeout", `命令超时（${Math.round((options?.timeoutMs ?? STATUS_TIMEOUT_MS) / 1000)}s）已终止，请缩小范围后重试`);
		}
...
/** Serializes .gitignore read-modify-write cycles (see the gitignore case). */
let ignoreWriteChain = Promise.resolve();
...
async function dispatch(endpoint, payload, env) {
	...
	switch (endpoint) {
		...
		case "diff": {
			...
			return ok({
				text,
				truncated: result.value.stdoutCapped === true
			});
		}
		case "log": {
			...
			// The subject is FREE TEXT: a 0x1f inside it splits extra fields,
			// so everything past field 7 is rejoined into the subject.
			const SEP = "\x1f";
			const result = await gitRun(gitPath, repo, [
				"log", "-n", String(count),
				"--date=format-local:%Y-%m-%d %H:%M",
				`--pretty=format:%h${SEP}%H${SEP}%an${SEP}%ae${SEP}%ad${SEP}%d${SEP}%P${SEP}%s`
			...
			const lines = result.value.stdout.split(/\r?\n/).filter(Boolean).map((row) => {
				...
				return {
					...
					subject: parts.length > 7 ? parts.slice(7).join(SEP) : ""
				};
			}).filter((e) => e.h && e.hash);
			return ok({ lines });
		}
		case "show": {
			...
			// `target` sits BEFORE `--`, so it must be rev-shaped — never a
			// git option (e.g. `--output=` would write files).
			const target = typeof payload.target === "string" ? payload.target.trim() : "";
			if (!target) return fail("git-error", "未指定要查看的提交");
			if (!isValidRev(target)) return fail("invalid-rev", "非法的提交标识");
			if (typeof payload.path === "string" && payload.path.trim() !== "") {
				const result = await gitRun(gitPath, repo, ["show", "--no-color", "--format=", target, "--", payload.path.trim()], { signal, lockFree: true });
				if (!result.ok) return result;
				const text = result.value.stdout;
				return ok({ text, truncated: result.value.stdoutCapped === true, files: [] });
			}
		...
		case "branches": {
			...
			// Fields are joined by 0x1f: refnames cannot contain ANY ASCII
			// control byte (git check-ref-format), so the split is exact even
			// for names containing `|` or other punctuation.
			const BSEP = "\x1f";
			const result = await gitRun(gitPath, repo, ["for-each-ref", `--format=%(HEAD)${BSEP}%(refname:short)${BSEP}%(upstream:short)${BSEP}%(objectname:short)`, "refs/heads", "refs/remotes"], { signal, lockFree: true });
			if (!result.ok) return result;
			const branches = result.value.stdout.split(/\r?\n/).filter(Boolean).map((line) => {
				const [marker, name, upstream, sha] = line.split(BSEP);
				return {
					name,
					current: marker === "*",
		...
		case "checkout": {
			...
			// Both `branch` and `start` precede `--` in argv: they must be
			// rev-shaped, never option-looking strings.
			if (!isValidRev(branch)) return fail("invalid-rev", "非法的分支名");
			const args = ["checkout"];
			if (payload.create === true) {
				args.push("-b", branch);
				if (typeof payload.start === "string" && payload.start.trim() !== "") {
					const start = payload.start.trim();
					if (!isValidRev(start)) return fail("invalid-rev", "非法的起点分支或提交");
					args.push(start);
				}
			} else {
				args.push(branch);
		...
		case "rename": {
			...
			if (!isValidRev(branch)) return fail("invalid-rev", "非法的分支名");
			if (!isValidRev(name)) return fail("invalid-rev", "非法的新分支名");
		...
		case "gitignore": {
			...
			// Serialize the read-modify-write: two concurrent gitignore RPCs
			// would race and the second write would silently drop the first's
			// line. The chain swallows errors (they become fail results) so it
			// can never poison later calls.
			const runIgnoreUpdate = async () => {
				let content = "";
			...
			const run = ignoreWriteChain.then(runIgnoreUpdate, runIgnoreUpdate);
			ignoreWriteChain = run.catch(() => { /* never poison the chain */ });
			return run;
		}
		case "list": {
			...
			try {
				...
				const walk = async (target, rel) => {
					...
					for (const dirent of dirents) {
						...
						if (UPLOAD_TEMP_RE.test(dirent.name)) { skipped++; continue; }   // live/orphaned upload temp
...
function writeRuntimeFileAtomic(entry) {
	...
	try {
		// 0600: the file carries the sidecar token; the base dir is
		// user-private (runtimeBaseDir), the mode is defense-in-depth.
...
async function handleVendor(req, res, url) {
	...
	// Stream with a size header instead of buffering the whole file: this
	// route is unauthenticated (loopback-only), and several workers weigh
	// multiple MB — a local process spamming requests must not multiply
	// that into unbounded resident memory. A small concurrency cap backs
	// the same goal.
	let st;
	try {
		st = await stat(abs);
	...
	if (activeVendor >= MAX_CONCURRENT_VENDOR) {
	...
	try {
		...
		/* client hung up mid-asset or read error: nothing to salvage */
	} finally {
		activeVendor--;
	}
}
...
// Exports live under the user-private base dir (NOT shared tmpdir — a
// pre-planted symlink there would let other local users harvest workspace
// archives or aim sweepExports' unlinks at their chosen directory).
const EXPORT_DIR = join(runtimeBaseDir(), "exports");
...
/**
 * Idle-timer touch for long byte transfers: re-arm at most once per second
 * so a >30min slow link no longer gets killed mid-upload by the idle exit
 * (each re-arm costs one clearTimeout+setTimeout — negligible at 1Hz).
 */
...
async function handleDownload(req, res, url, config) {
	if (activeTransfers >= MAX_CONCURRENT_TRANSFERS) {
		sendJson(res, 503, fail("busy", "传输任务过多，请稍后重试"));
		return;
	}
	const touch = makeIdleTouch();
	...
	if (abs !== "") {
		...
		// Same repo resolution as the RPC channel (defaultRoot fallback) —
		// an omitted repo used to silently fence the download to the
		// sidecar's own cwd, which only ever produced confusing errors.
		const repoRes = resolveRepo({ repo: q.get("repo") ?? "" }, config.defaultRoot);
		if (!repoRes.ok) {
			sendJson(res, 400, fail(repoRes.error.code, repoRes.error.message));
			return;
		}
		const resolved = resolveUnder(repoRes.repo, q.get("path") ?? "");
		if (!resolved.ok) {
	...
	try {
		...
		rs.on("data", touch); // long downloads must not hit the idle exit
		res.on("close", () => { try { rs.destroy(); } catch { /* already done */ } });
		await pipeline(rs, res);
	} catch (error) {
...
async function handleUpload(req, res, url, config) {
	if (activeTransfers >= MAX_CONCURRENT_TRANSFERS) {
		sendJson(res, 503, fail("busy", "传输任务过多，请稍后重试"));
		return;
	}
	...
	const cap = config.caps?.uploadBytes ?? DEFAULT_MAX_UPLOAD_BYTES;
	...
	const abortUpload = () => {
		rejected = true;
		// Clean the temp only AFTER the write stream has fully closed: on
		// Windows the fd release behind ws.destroy() is asynchronous, and an
		// immediate unlinkSync can race the still-open handle (EBUSY/EPERM —
		// silently swallowed by unlinkExportFile) leaving the .dshup-* temp
		// behind as litter. The catch-path unlink below stays as a belt-and-
		// braces second attempt (no-op where this one already won).
		ws.once("close", () => unlinkExportFile(temp));
	...
	req.on("data", (chunk) => {
		...
		touch(); // body progress counts as activity (slow links can exceed 30min)
	...
	try {
		...
		if (existed || overwrite) {
			...
			// No-clobber publish: hardlink + unlink is atomic AND fails with
			// EEXIST when the target appeared between the 409 check and now
			// (a bare rename would silently clobber it — the TOCTOU the code
			// review flagged). Filesystems without hardlink support fall back
			// to rename, keeping that residual race.
			try {
...
function handleEvents(req, res, url, config) {
	...
	const repoRes = resolveRepo({ repo: url.searchParams.get("repo") ?? "" }, config.defaultRoot);
	if (!repoRes.ok) {
...
function readBody(req, cap) {
	return new Promise((resolveBody) => {
		...
		req.on("error", () => {
			finish({ body: null, tooLarge: false });
			// Tear the socket down too: without this the peer can leave the
			// connection hanging until the server timeout reaps it.
			try { req.destroy(); } catch { /* already gone */ }
		});
	});
}
...
async function handleRequest(req, res, config) {
	...
	try {
		...
		// Malformed request-target (`GET //:80`, absolute-form with an
		// out-of-range port, …): WHATWG URL throws on these BEFORE the token
		// gate below. This must answer 400 — never become an unauthenticated
		// unhandled-rejection crash of the whole service.
	...
	if (req.method === "GET" && url.pathname === "/files/download") {
		await handleDownload(req, res, url, config);
		return;
	}
	if (req.method === "POST" && url.pathname === "/files/upload") {
		await handleUpload(req, res, url, config);
		return;
	}
...
async function main() {
	...
	// Belt-and-suspenders: a long-lived local helper must log stray
	// unhandled rejections / exceptions and keep serving instead of dying —
	// the host would respawn us, but in-flight work would be lost. (The
	// request-target parser above answers 400 instead of throwing, so these
	// should stay quiet in practice.)
	process.on("unhandledRejection", (reason) => {
		svcLog(`unhandledRejection: ${String(reason?.stack ?? reason).slice(0, 300)}`);
	});
	process.on("uncaughtException", (error) => {
		svcLog(`uncaughtException: ${String(error?.stack ?? error).slice(0, 300)}`);
	});
	...
	server.listen(0, "127.0.0.1", () => {
		...
		// Boot hygiene: archives orphaned by a crashed predecessor would
		// otherwise linger until the next export happens to sweep.
		sweepExports();
		armIdleTimer();
	});
}
