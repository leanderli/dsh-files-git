...
import { appendFileSync, createReadStream, createWriteStream, existsSync, linkSync, mkdirSync, readFileSync, readdirSync, realpathSync, renameSync, statSync, unlinkSync, watch, writeFileSync } from "node:fs";
 ...
 *  `<name>.dshup-<hex>`. A crashed sidecar orphans them; listings sweep
 *  stale ones and hide all of them, exports skip them. */
	...
	switch (endpoint) {
		...
		case "log": {
			...
			const lines = result.value.stdout.split(/\r?\n/).filter(Boolean).map((row) => {
				const [h, hash, name, email, date, refs, parents, subject] = row.split(SEP);
				return { h, hash, name: name || "", email: email || "", date: date || "", refs: (refs || "").trim(), parents: (parents || "").split(" ").filter(Boolean), subject: subject || "" };
			}).filter((e) => e.h && e.hash);
			return ok({ lines });
		}
		case "show": {
